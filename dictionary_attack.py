...
if __name__ == "__main__":
    ...
    for file_index, wordlist in enumerate(wordlists):
        try:
            display(':', f"Loading File {Back.MAGENTA}{file_index} => {wordlist}{Back.RESET}", start='\n')
        ...
        display(':', f"\tTime Taken = {t2-t1:.2f} seconds")
        display(':', f"\tRate = {len(words)/(t2-t1):.2f} hashes/second")
    ...
    display(':', f"Total Hashes Calculated = {Back.MAGENTA}{len(hashes_calculated)}{Back.RESET}")
